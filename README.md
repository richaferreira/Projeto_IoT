...
│   ├── guia-uso-e-testes.md           # Guia de uso no navegador e relatorio de testes
│   └── imagens/
│       ├── montagem-circuito-01.jpg
│       ├── montagem-circuito-02.jpg
│       └── montagem-circuito-03.jpg
└── frontend/
    ├── index.html            # Dashboard web (simulacao)
    ├── serial.html           # Dashboard web (conexao real via Web Serial API)
    ├── style.css             # Estilos compartilhados
    ├── app.js                # Logica de simulacao (espelho do codigo.ino)
    └── serial.js             # Logica de conexao serial com Arduino real
```
...
### Conexao com Arduino Real (Web Serial API)

Além da simulacao, o projeto inclui uma versao do dashboard que conecta diretamente ao Arduino via **Web Serial API**:

```bash
# Abrir a versao com conexao serial
open frontend/serial.html
```

**Requisitos:**
- Google Chrome 89+ ou Microsoft Edge 89+
- Arduino conectado via USB
- O codigo `codigo.ino` deve estar rodando no Arduino (saida serial a 9600 baud)

**Como usar:**
1. Abra `serial.html` no Chrome
2. Clique em "Conectar ao Arduino"
3. Selecione a porta serial do Arduino na janela do navegador
4. O dashboard passa a exibir os dados reais do sensor de chama

> A versao serial le e interpreta automaticamente a saida do `Serial.print()` do Arduino (`Sensor: XXX | Estado: YYY`) e atualiza o gauge, grafico, LEDs e estado em tempo real.
